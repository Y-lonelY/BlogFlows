...
用于输入时间和日期的控件，[link](https://developer.apple.com/documentation/uikit/uidatepicker)

`class UIDatePicker : UIControl` 继承至 UIControl

在视图中添加时间选择器：

1. 在创建时设置日期选择器模式
2. 如果需要，提供其他配置选项，例如最小和最大日期
3. 将操作方法​​连接到日期选择器
4. 设置自动布局规则以控制界面中日期选择器的位置

日期选择器采用 Target-Action 的设计模式，这意味可以使用 `addTarget(_:action:for:)` 来对其事件进行监听
...
datePicker.locale = Locale(identifier: "zh_CN") // 设置时区为中国

datePicker.date = Date(timeIntervalSinceNow: -60*60*24*7) // 设置默认展示时间，为第前七天

datePicker.datePickerMode = .date // 设置时间选择器的选择模式

datePicker.maximumDate = Date() // 设置最大时间为当前时间

datePicker.isHidden = true // 设置其隐藏
```