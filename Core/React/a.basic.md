...
- [JSX](#jsx)
- [State](#state)
	- [state的一些冷知识](#state%E7%9A%84%E4%B8%80%E4%BA%9B%E5%86%B7%E7%9F%A5%E8%AF%86)
- [组件](#%E7%BB%84%E4%BB%B6)
	- [受控组件和非受控组件](#%E5%8F%97%E6%8E%A7%E7%BB%84%E4%BB%B6%E5%92%8C%E9%9D%9E%E5%8F%97%E6%8E%A7%E7%BB%84%E4%BB%B6)
	- [class 组件转换为 function](#class-%E7%BB%84%E4%BB%B6%E8%BD%AC%E6%8D%A2%E4%B8%BA-function)
...
getDerivedStateFromProps(props, state)是一个静态方法，因此不能使用 this，其有两个参数，返回一个对象来更新当前的 state，如果不需要更新则返回 null，尽量不要使用该方法，会造成组件难以维护
...
class Test extends React.Component {
	...
	/**
	 * 在 render() 方法之前调用，并且在挂载和更新时都会调用
	 * 该生命周期存在的唯一目的就是：使用派生 state 时，即当 props 改变时，引起 state 的变化
	 * 派生状态会导致代码冗余，并使组件难以维护，因此一般都使用替代方案
	 */
	...
	/**
	 * 手动判断当前组件是否需要更新
	 * 当 props 和 state 发生改变时调用，挂载阶段和 forceUpadte() 不会触发此生命周期
	 * 此方法仅作为性能优化的方式存在，不要试图通过此方法来阻止渲染
	 * return false 并不会阻止子组件在 state 更新时重新渲染
	 */
...
在一些情况下，希望组件不被渲染，一个办法就是：在 `render()` 方法内通过 `return null;` 来实现，但是需要注意：
组件在 `render()` 方法内返回 null 并不会影响组件声明周期方法的回调，即生命周期方法仍可以被调用
